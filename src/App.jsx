...
const BENCHMARKS = [
  { key: "wig", label: "WIG", symbol: "wig" },
  { key: "wig20", label: "WIG20", symbol: "wig20" },
  { key: "wig40", label: "WIG40 (mWIG40)", symbol: "mwig40" },
...
async function fetchStooqSeries(symbol, d1, d2) {
  ...
  const proxyUrl = `/.netlify/functions/stooq-proxy?${query}`;
...
export default function App() {
  ...
  const refreshPrices = useCallback(async () => {
    ...
    let usdPlnRate = null;
    try {
      const rows = await fetchStooqSeries("usdpln");
    ...
    const results = await Promise.all(
      remaining.map(async (h) => {
        try {
          const q = await fetchStooqQuote(h.ticker, usdPlnRate);
          return { ticker: h.ticker, quote: { ...q, source: "stooq", fetchedAt: Date.now() } };
        } catch (e) {
          if (!stooqErrorMsg) stooqErrorMsg = `${h.ticker}: ${String(e.message || e)}`;
        }
        try {
    ...
    let stooqCount = 0, aiCount = 0;
    const failedTickers = [];
    for (const r of results) {
      if (r.quote) {
        next[r.ticker] = r.quote;
        if (r.quote.source === "stooq") stooqCount++;
        else aiCount++;
      } else {
    ...
    parts.push(`Stooq: ${stooqCount}/${remaining.length}${stooqCount === 0 && stooqErrorMsg ? ` (${stooqErrorMsg})` : ""}`);
...
function StatystykiTab({ transactions, portfolios, prices }) {
  ...
  async function runComparison() {
    ...
    for (const key of selectedBenchmarks) {
      ...
      try {
        const rows = await fetchStooqSeries(bench.symbol, earliest, new Date());
    ...
    <div className="max-w-md mx-auto px-4 pt-6">
      ...
      <div className="rounded-2xl bg-slate-900 border border-slate-800 p-4 mb-6">
        ...
        <p className="text-xs text-slate-600 mt-3">
          Zwrot od najwcześniejszej transakcji do dziś. Wymaga zewnętrznych danych giełdowych (Stooq) — w środowisku Claude może się nie udać z powodu ograniczeń sieciowych.
        </p>
      </div>
    </div>
  );
    ...
    (async () => {
      ...
      try {
        if (timeframe === "1d") {
          ...
          const d2 = new Date();
          const tf = TIMEFRAMES.find((t) => t.key === timeframe);
          const d1 = tf && tf.days ? new Date(d2.getTime() - tf.days * 24 * 60 * 60 * 1000) : cutoffDate;
          const stooqSymbol = ticker.toUpperCase().endsWith(".PL") ? ticker.replace(/\.PL$/i, "") : ticker;
          const rows = await fetchStooqSeries(stooqSymbol, d1, d2);
